...
|---|---|---|---|---|---|
| **1.1.1** | Non-text Content | A | Yes | ✅ Pass | 16 decorative `<svg>`s now carry `aria-hidden="true"`; car render has `alt="Volkswagen ID.3 Neo"`. **0 unnamed nodes** in the AX tree, all 5 viewports — axe/WAVE/Nu all missed the original defect. |
...
| SC | Name | Lvl | Relevant | Status | Evidence / what to do |
|---|---|---|---|---|---|
| **1.3.1** | Info and Relationships | A | Yes | ✅ Pass | One `h1`, `role="banner"` topbar, `main`; 3 named `<select>`s, 2 labelled switches (`speed-toggle`, `ac-toggle`), 1 labelled radiogroup (occupancy). axe: 0 violations, 98 rules. |
| **1.3.2** | Meaningful Sequence | A | Yes | ✅ Pass* | DOM order matches visual order; 18 Tab stops follow the question sequence top to bottom. |
...
| **1.4.4** | Resize Text | AA | Yes | ✅ Pass | **0 violations, no horizontal scroll, all 18 controls present+reachable** at 400% zoom (320×256 @ dsf 4) — past the 200% required. |
...
| **1.4.11** | Non-text Contrast | AA | Yes | ✅ Pass | `.fl-select select` border `rgb(110,116,126)` = 4.32:1, clears 3:1 outright (real core uses `rgb(161,164,172)`=2.29:1 — **deliberate deviation: this prototype passes outright regardless of the upstream component**). Focus ring `rgb(200,108,3)` ≈3.75:1 vs white/page bg. |
| **1.4.12** | Text Spacing | AA | Yes | ✅ Pass | All 4 overrides at 1440/390/320: no clipping, no lost control, no scroll. `.select-group` stacks selects vertically (unconditional), giving floating labels full row width incl. 960–1024px; matching `<optgroup label>` as backup. |
...
| SC | Name | Lvl | Relevant | Status | Evidence / what to do |
|---|---|---|---|---|---|
| **2.1.1** | Keyboard | A | Yes | ✅ Pass | All 18 controls keyboard-operable: `speed-toggle`/`ac-toggle` flip via real `Space`; occupancy radiogroup moves via arrows (no `Space` needed); selects/temp-slider via arrows; both `role="slider"` thumbs via arrows (`Shift`=bigger step). |
| **2.1.2** | No Keyboard Trap | A | Yes | ✅ Pass | **No trap.** Tab cycles all 18 stops, returns to first. Modal open: Tab cycles its own 2 stops (body, close); Escape/backdrop-click always exits. |
...
| **2.4.3** | Focus Order | A | Yes | ✅ Pass | 18 Tab stops, DOM order = visual order, verified at 1440×900/390×844 with real Tab. Any of the 7 info triggers moves focus to the shared modal body, returns it to the trigger on close. |
...
| **2.4.7** | Focus Visible | AA | Yes | ✅ Pass | **All 18 stops + the modal's 2 stops show a visible ring**: `outline: 2px solid var(--focus-orange)` (`rgb(200,108,3)`), `outline-offset:0`. On the 2 hidden-checkbox switches + occupancy radiogroup, drawn on the ancestor label via `:has(input:focus-visible)`, firing on the visible surrogate not the 1×1 clipped input. |
...
| **2.5.3** | Label in Name | A | Yes | ✅ Pass | `#occ-1p`/`#occ-full`, each in its own `<label>` — "1 person"/"Full" are each radio's *entire* visible label AND accessible name, verbatim (no `aria-label` override). Old single-switch design was an arguable pass; splitting to 2 radios removed the ambiguity outright. |
| **2.5.4** | Motion Actuation | A | No | ⚪ N/A | No device-motion or user-motion actuation. |
| **2.5.7** | Dragging Movements | AA | Yes | ✅ Pass | The two `role="slider"` distribution thumbs support drag, but also arrow keys (`Shift`=bigger step) and click-anywhere-on-track — dragging is never required. Native temp `<input type=range>` gets the same guarantee free. |
| **2.5.8** | Target Size (Minimum) | AA | Yes | ✅ Pass | **1 real gap found+fixed this pass.** Switch inputs are 1×1 but target = wrapping `label.vw-switch`, 60×24. Occupancy's `label.vw-toggle-opt` was ~20px tall (test selector `label.vw-toggle` didn't match real class) — fixed: `min-height:24px` → measured 59.56×24 / 25.16×24, selector corrected. Both `role="slider"` thumbs ray-cast ≥24×24 at every viewport incl. 320×256@dsf4. |
...
| **4.1.2** | Name, Role, Value | A | Yes | ✅ Pass | `#trim-select`'s name = question + "Model: " + current trim group, updates on change (verified via CDP `getPartialAXTree` before/after `selectOption`). `#battery-select` disables via native `disabled` when trim has 1 battery option (e.g. Trend), re-enables on multi-option — verified both directions in `tests/structural.spec.js`. **AX tree: 245 nodes, 34 named, 0 unnamed, 0 duplicate role+name**; the 18 Tab-reachable controls independently re-verified this pass, all named (the 245-node count itself not re-verified — see "Decisions" below). |
...
**No decisions left to record** — the one prior decision (SC 2.5.3, occupancy) was resolved by
code, not re-argued: 2 native radios each named by its own visible label removed the ambiguity
outright. See the SC 2.5.3 row above, `a11y-3-implementation.md` §7.

**One real defect found+fixed this pass:** SC 2.5.8 — occupancy radiogroup's 2 click targets were
~20px tall (under 24px floor), untested because the suite's selector didn't match the real CSS
class. Both CSS and test fixed — see the SC 2.5.8 row above.
...
a dialog trigger or radiogroup) — no markup changes needed. **NVDA 2026.1.1.55980 remains the one
gap** (VoiceOver is a deviation, not a substitute) — required before formal sign-off.

# Decisions an auditor could challenge

24 of the 56 A/AA criteria have **no machine-testable ACT rule**, several applying directly here
(1.4.11, 1.4.13, 2.5.1, 2.5.2, 2.5.8, 2.4.11) — "passes" reflects a **judgement** backed by this
app's own Playwright evidence (ray-casting, computed-style assertions), not a formal ACT result.

**Not re-verified this pass:** SC 4.1.2's "245 nodes, 34 named, 0 unnamed, 0 duplicate" AX-tree
snapshot — a raw walk this pass returned 249 (likely different counting rules, not an error). The
normative claim (0 unnamed, no duplicate name+role) is independently confirmed by the passing test
suite regardless. Node-count methodology reconciliation left open.
