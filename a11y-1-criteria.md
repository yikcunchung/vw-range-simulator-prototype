...
| **1.4.11** | Non-text Contrast | AA | Yes | ✅ Pass | `.fl-select select`'s border is `rgb(110,116,126)` (4.32:1), clearing 3:1 outright — no exception argument needed. This is a deliberate deviation from the real production core Select component (which uses `rgb(161,164,172)`, 2.29:1, verified by pixel-sampling a live screenshot): **this prototype's purpose is to demonstrate a build that passes every criterion outright, regardless of whether the upstream core component itself does.** The unified focus ring is `#C86C03` (`--focus-orange`), computed `rgb(200, 108, 3)`, ≈3.75:1 against both white and the page background — also clears 3:1. |
